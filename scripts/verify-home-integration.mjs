...
try{for(const width of [1280,375]){
 ...
 await page.evaluate(()=>MinihompyApp.renderView('board'));await page.locator('.board-write').click();await page.locator('#board-edit-title').fill('DRAFT');accept=false;const before=page.url();await page.locator('[data-menu=photos]').click();assert.equal(page.url(),before);assert.equal(await page.locator('#board-edit-title').inputValue(),'DRAFT');accept=true;await home();
 ...
 console.log(`PASS ${width}: draft cancellation, history/reload, same-browser tabs, blocked storage, server day and dedup cleanup preserve TOTAL`);
