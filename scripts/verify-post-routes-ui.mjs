...
const setup=`
 ...
 window.MinihompyBoardRepository={...base};
...
try{
 ...
 await page.evaluate(()=>MinihompyApp.renderView('board'));await page.locator('.board-write').waitFor();await page.locator('.board-write').click();await page.locator('#board-edit-title').fill('UNSAVED');accept=false;const before=page.url();await page.locator('[data-menu=photos]').click();assert.equal(page.url(),before);assert.equal(await page.locator('#board-edit-title').inputValue(),'UNSAVED');
 await page.evaluate(()=>history.back());await page.waitForTimeout(150);assert.equal(page.url(),before);assert.equal(await page.locator('#board-edit-title').inputValue(),'UNSAVED');accept=true;await page.locator('[data-menu=photos]').click();await page.locator('.photo-post').waitFor();assert.ok(dialogs>=3);
 console.log('PASS '+width+': back/forward and canceled tab/history navigation retain URL, view and draft');
