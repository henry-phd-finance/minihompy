...
try{
 ...
 const out=process.env.VERIFICATION_DIR||'docs/verification/friend-visibility-step8';await mkdir(out,{recursive:true});
 try{for(const width of [1280,375]){
 const page=await browser.newPage({viewport:{width,height:820}}),errors=[];page.on('pageerror',e=>errors.push(e.message));let renews=0,hold=false,release,offline=false;
 const h=homes[2],files=['member-writing-client.js','member-writing-runtime.js','content-access.js','home-repository.js','home-activity.js'];
 await page.route('**/*',async route=>{
  const req=route.request(),url=new URL(req.url());
  if(req.url().startsWith(h.config.SUPABASE_URL)){
   if(url.pathname.endsWith('/renew'))renews++;
   if(offline&&url.pathname.endsWith('/summary'))return route.fulfill({status:503,json:{error:{code:'IDENTITY_UNAVAILABLE'}}});
   const res=await handleMemberWriting(new Request(req.url(),{method:req.method(),headers:req.headers(),...(req.postData()?{body:req.postData()}:{})}),{config:h.config,fetcher,db:h.db});
   const payload={status:res.status,headers:Object.fromEntries(res.headers),body:await res.text()};
   if(hold&&url.pathname.endsWith('/summary')){hold=false;await new Promise(r=>release=r);}return route.fulfill(payload).catch(()=>{});
  }
  ...
  const setup=`window.MINIHOMPY_SUPABASE={url:${JSON.stringify(h.config.SUPABASE_URL)}};window.MINIHOMPY_VISITOR_IDENTITY_CONFIG={enabled:true,siteId:${JSON.stringify(site(2))}};window.MINIHOMPY_MEMBER_WRITING_CONFIG={enabled:true};window.MINIHOMPY_HOME_DATA_CONFIG={enabled:true,supabaseUrl:MINIHOMPY_SUPABASE.url,homepage:'https://m2.test/'};window.MINIHOMPY_CONFIG={menus:['board','photos','diary','guestbook'].map(id=>({id,visible:true}))};window.MinihompySharedIdentity={state:{status:'identified',visitor:{id:${JSON.stringify(member(1))}}}};window.MinihompyAdmin={state:{role:'reader',userId:null}};window.MinihompyVisitorSession={context:async()=>({role:MinihompyAdmin.state.role,client:{auth:{getSession:async()=>({data:{session:{access_token:'owner.valid.jwt'}}})}}})};window.MinihompyPostRoutes={href:(kind,id)=>'#/'+kind+'?post='+id};window.setUser=(n,data)=>{const key='minihompy.member-writing.v1:'+MINIHOMPY_VISITOR_IDENTITY_CONFIG.siteId+':'+MINIHOMPY_SUPABASE.url;sessionStorage.setItem(key,data.session_token);sessionStorage.setItem(key+':renewal',data.renewal_token);sessionStorage.setItem('minihompy.writing.pending:'+MINIHOMPY_VISITOR_IDENTITY_CONFIG.siteId+':member',data.actor.member_id);MinihompySharedIdentity.state={status:'identified',visitor:{id:data.actor.member_id}};dispatchEvent(new Event('minihompy:identity'));dispatchEvent(new Event('minihompy:visitor-identity'));};setUser(1,${JSON.stringify(sessions['1:2'])});`;
  return route.fulfill({contentType:'text/html',body:'<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><link rel="stylesheet" href="styles.css"><span id="member-session-status"></span><button id="member-session-retry" hidden>인증 재시도</button><div class="home-activity"></div><script>'+setup+'</script>'+files.map(f=>'<script src="'+f+'"></script>').join('')+'<script>MinihompyHomeActivity.attach(document.querySelector(".home-activity"));</script>'});
 });
 const ready=()=>page.waitForFunction(()=>document.querySelector('.home-activity')?.dataset.status==='ready');
 const total=()=>page.locator('.home-activity [data-kind=board] dd').innerText();
 await page.goto('https://m2.test/');await ready();assert.ok((await total()).endsWith('/ 2'));assert.equal(await page.getByText('회원 확인',{exact:true}).count(),0);
 await page.screenshot({path:out+'/member-'+width+'.png'});console.log('PASS '+width+': actual runtime/repository/home shows friend data without confirmation button');
 // Force the local access token into the automatic renewal window; renewal credential remains valid.
 await h.pg.query("update private.member_writing_sessions set expires_at=clock_timestamp()+interval '20 seconds' where token_hash=$1",[await tokenHash(sessions['1:2'].session_token)]);
 await page.evaluate(()=>dispatchEvent(new Event('focus')));await ready();await page.waitForFunction(()=>MinihompyMemberWriting.state.status==='ready');assert.ok(renews>=1);
 console.log('PASS '+width+': shared runtime renews an expiring session automatically before home read');
 hold=true;await page.evaluate(()=>dispatchEvent(new Event('focus')));for(let i=0;!release&&i<100;i++)await page.waitForTimeout(20);assert.ok(release);
 await page.evaluate(data=>setUser(3,data),sessions['3:2']);await ready();assert.ok((await total()).endsWith('/ 1'));release();release=null;await page.waitForTimeout(50);assert.ok((await total()).endsWith('/ 1'));
 console.log('PASS '+width+': A to C switch clears protected results and rejects late A response');
 offline=true;await page.evaluate(()=>dispatchEvent(new Event('focus')));await page.locator('.home-activity-retry').waitFor();assert.equal(await page.locator('.home-post-link').count(),0);assert.equal(await page.locator('.home-activity-empty').count(),0);offline=false;
 await page.locator('.home-activity-retry').click();await ready();assert.ok((await total()).endsWith('/ 1'));
 await page.evaluate(()=>{MinihompyAdmin.state={role:'admin',userId:'local-owner'};dispatchEvent(new Event('minihompy:identity'));});await ready();assert.ok((await total()).endsWith('/ 3'));
 await page.evaluate(()=>{MinihompyAdmin.state={role:'reader',userId:null};MinihompySharedIdentity.state={status:'anonymous'};dispatchEvent(new Event('minihompy:identity'));dispatchEvent(new Event('minihompy:visitor-identity'));});await ready();assert.ok((await total()).endsWith('/ 1'));
 console.log('PASS '+width+': failure never becomes empty/public, retry recovers, owner sees private and logout clears it');
 ...
 console.log('All 8 browser friend home scenarios passed using actual runtime, repository, central/personal handlers and SQL; Auth/identity display and readiness are fixtures.');
