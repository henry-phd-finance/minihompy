...
try{
 ...
 for(const width of [1280,375]){
  ...
  const refresh=async()=>{if(await action('refresh').count())await action('refresh').click();else await page.evaluate(()=>dispatchEvent(new Event('focus')));};
  const ready=()=>page.waitForFunction(()=>{const b=document.querySelector('[data-review-action="save"]');return b&&!b.hidden&&!b.disabled;});
  await ready();assert.equal(await items.count(),0);assert.equal(await action('refresh').count(),0);assert.equal(await page.locator('.friend-reviews .friends-heading').innerText(),'일촌평');assert.equal(await action('save').innerText(),'확인');assert(!(await page.locator('.friend-reviews').innerText()).includes('등록된 일촌평'));
  assert(await page.locator('.friend-review-form').evaluate(e=>{const h=e.querySelector('h2').getBoundingClientRect(),t=e.querySelector('textarea').getBoundingClientRect(),b=e.querySelector('button').getBoundingClientRect();return h.right<t.left&&t.right<b.left&&Math.abs(h.y+h.height/2-t.y-t.height/2)<1;}));
  ...
  listError=true;await refresh();await page.waitForFunction(()=>document.querySelector('.friend-reviews').textContent.includes('일촌평을 불러오지 못'));listError=false;await action('refresh').focus();await page.keyboard.press('Enter');await page.waitForFunction(()=>document.querySelectorAll('.friend-reviews li').length===5);
  ...
  await accept();await page.evaluate(()=>MinihompyMemberWriting.retry());await refresh();await ready();await input.fill('메뉴 나가면 폐기');await page.evaluate(()=>document.querySelector('[data-view-slot="main"]').replaceChildren());await mount();await ready();assert.equal(await input.inputValue(),'');await input.fill('다른 계정에 남지 않음');
  hold=true;await refresh();while(!release)await new Promise(r=>setTimeout(r,10));
  ...
  await refresh();await page.waitForFunction(()=>document.querySelectorAll('.friend-reviews li').length>0);await page.evaluate(()=>{const owner={id:MINIHOMPY_VISITOR_IDENTITY_CONFIG.siteId,handle:'self',display_name:'내 홈'};MinihompySharedIdentity.state={status:'identified',visitor:owner};window.MinihompyNavigation={state:{owner,visitor:owner,status:'self'}};dispatchEvent(new CustomEvent('minihompy:navigation-state'));});assert(!(await page.locator('.friend-reviews').innerText()).includes('자신의 홈'));assert.equal(await action('refresh').count(),0);assert.equal(await action('save').isHidden(),true);assert.deepEqual(errors,[]);
