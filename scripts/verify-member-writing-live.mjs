// Explicit production test: only UUIDs created by this run may be removed.
...
const [playwright]=process.argv.slice(2);if(!playwright||process.env.MINIHOMPY_LIVE_WRITING!=='1')throw Error('Explicit MINIHOMPY_LIVE_WRITING=1 and Playwright path required');
...
const run=crypto.randomUUID(),prefix='회원연결 검증 '+run,checks=[],contexts=[],created=[];let cleanupOk=false,ownerPage,failed=false;
...
try{
 ...
 await second.locator('#login-auth-toggle').click();await second.waitForURL('**/login.html?**');await second.locator('#handle').fill(b.handle);await second.locator('#submit').click();await second.waitForURL(url=>url.origin===new URL(b.home).origin&&url.pathname.endsWith('/login/'));await second.locator('#password').fill(b.password);await second.locator('#submit').click();await state(second,b,b);await second.locator('.guestbook-authorize').click();await second.locator('.guestbook-body-input').waitFor();assert.equal(await second.locator('.guestbook-body-input').inputValue(),'');pass('A logout → B login on the same browser retains no A draft');
 ...
 await browser.close();
 const report={checkedAt:new Date().toISOString(),run,checks,passed:!failed&&cleanupOk,testContentCleaned:cleanupOk,createdGuestbookIds:created};
 await writeFile(new URL('../docs/verification/member-writing-step7/live.json',import.meta.url),JSON.stringify(report,null,2)+'\n');
