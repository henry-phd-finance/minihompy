begin;
-- Guestbook: 10-second cooldown and the existing 20-write / 24-hour window.
-- Keep counters, authorization and idempotent retry records intact.
create or replace function public.member_guestbook(p_action text,p_args jsonb)
...
begin
  ...
  if p_action='list' then
    ...
    if p_args ? 'post' then
      resource:=(p_args->>'post')::uuid;
      select * into item from public.guestbook_posts g where id=resource and (visibility='public' or mode='owner' or (author_kind='member' and author_member_id=actor));
      if not found then return jsonb_build_object('failure','NOT_FOUND');end if;
      select (count(*)/size+1)::integer into page from public.guestbook_posts g where (visibility='public' or mode='owner' or (author_kind='member' and author_member_id=actor)) and (created_at,id)>(item.created_at,item.id);
    end if;
  ...
  if p_action='create' then
    ...
    if limits.writes>=20 then
      return jsonb_build_object('failure','RATE_LIMITED','limit_reason','guestbook_daily',
        'retry_after',greatest(1,ceil(extract(epoch from greatest(limits.window_start+interval '24 hours',limits.last_write+interval '10 seconds')-write_now)))::integer);
    end if;
    if limits.last_write>write_now-interval '10 seconds' then
      return jsonb_build_object('failure','RATE_LIMITED','limit_reason','guestbook_cooldown',
        'retry_after',greatest(1,ceil(extract(epoch from limits.last_write+interval '10 seconds'-write_now)))::integer);
    end if;
...
declare limits private.guestbook_write_limits; write_now timestamptz := clock_timestamp(); wait_minutes integer;
begin
  ...
  if tg_op='UPDATE' then
    ...
    if limits.window_start<=write_now-interval '24 hours' then limits.window_start:=write_now;limits.writes:=0;end if;
    if limits.writes>=20 then
      wait_minutes:=greatest(1,ceil(extract(epoch from greatest(limits.window_start+interval '24 hours',limits.last_write+interval '10 seconds')-write_now)/60))::integer;
      raise exception '방명록은 24시간 내에 최대 20개까지 작성할 수 있습니다. %시간 %분 후 다시 작성할 수 있습니다.',wait_minutes/60,wait_minutes%60;
    end if;
    if limits.last_write>write_now-interval '10 seconds' then
      raise exception '방명록은 10초에 1개씩 작성할 수 있습니다. %초 후 다시 작성할 수 있습니다.',greatest(1,ceil(extract(epoch from limits.last_write+interval '10 seconds'-write_now)))::integer;
    end if;
