...
-- Aggregate reads never mutate content or grant access to private guestbook rows.
create index if not exists photo_posts_home_recent on public.photo_posts(created_at desc,id desc);
create index if not exists diary_entries_home_recent on public.diary_entries(created_at desc,id desc);
create index if not exists guestbook_posts_home_public on public.guestbook_posts(created_at desc,id desc) where visibility='public';
create index if not exists post_comments_home_today on public.post_comments(created_at);

create or replace function private.home_summary_text(value text) returns text
language sql immutable set search_path='' as $$
 select left(btrim(regexp_replace(
   regexp_replace(regexp_replace(coalesce(value,''), '<[^>]*>', ' ', 'g'),
   E'\\[([^]]*)\\]\\([^)]*\\)', E'\\1', 'g'),
   '[[:space:]#*_`~]+', ' ', 'g')),120)
$$;
revoke all on function private.home_summary_text(text) from public,anon,authenticated,service_role;

-- Private clock seam permits deterministic boundary tests. Not an exposed RPC.
...
begin
 ...
 with posts as materialized (
   -- These three schemas currently store public posts only. Visibility changes
   -- in backlog 4 must extend these predicates together with their RLS policies.
   select 'board'::text kind,id,created_at from public.board_posts where 'board'=any(menus) and created_at<=p_as_of
   union all select 'photos',id,created_at from public.photo_posts where 'photos'=any(menus) and created_at<=p_as_of
   union all select 'diary',id,created_at from public.diary_entries where 'diary'=any(menus) and created_at<=p_as_of
...
create or replace function public.home_summary(p_menus text[] default array['board','photos','diary','guestbook'])
returns jsonb language sql stable security definer set search_path='' as $$
 select private.home_summary_at(p_menus,statement_timestamp())
$$;
revoke all on function public.home_summary(text[]) from public,anon,authenticated,service_role;
grant execute on function public.home_summary(text[]) to anon,authenticated;
comment on function public.home_summary(text[]) is 'Public-only home summary v1. Hidden menus excluded server-side; caller identity never expands results.';
commit;
