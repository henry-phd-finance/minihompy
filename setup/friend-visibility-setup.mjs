...
export const friendVisibilityMigrations=['202609240007_friend_visibility.sql','202609240008_friend_comments.sql','202609240009_friend_photo_media.sql','202609240010_friend_aggregates.sql','202609240011_friend_diary_filters.sql','202609240012_friend_visibility_deployment.sql'];
const literal=s=>"'"+String(s).replaceAll("'","''")+"'";
const markers=["to_regclass('private.friend_visibility_state') is not null","to_regprocedure('public.member_content_comments(text,jsonb)') is not null","to_regprocedure('public.member_photo_read(text,jsonb)') is not null","to_regprocedure('public.member_content_aggregate(text,jsonb)') is not null","position('day_date date' in coalesce(pg_get_functiondef(to_regprocedure('public.member_content_read(text,jsonb)')),''))>0","to_regclass('private.friend_visibility_deployment') is not null"];
